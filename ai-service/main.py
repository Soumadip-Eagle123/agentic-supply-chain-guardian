...
5. Estimate 'estimated_delay_hours' as a numeric float based on:
   - Road surface condition (potholes/damage adds 1-4 hrs, standard-paved adds 0)
   - Weather severity (storms/flooding adds 2-6 hrs, clear adds 0)
   - Cargo sensitivity (hazardous/perishable cargo needing rerouting adds 1-3 hrs)
   - If risk is "Low" and no disruptions found, set this to 0.0.
6. Construct 'dynamic_eta' as a short string combining delay + cause, e.g. "+3.5 hrs (Flash Flood Detour)" or "On schedule" if 0.0.
